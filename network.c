...
void forward_network(network net, network_state state)
{
    ...
    for(i = 0; i < net.n; ++i){
        ...
        //从这里开始就一层一层地分析了，[convolutional],[maxpool],[local],[dropout],[connected],[detection]
        l.forward(l, state);
...
float get_network_cost(network net)
{
    ...
    int count = 0;
    for(i = 0; i < net.n; ++i){
        //获取各层的损失，可搜索关键词l.cost了解，只有detection层有cost，这个函数在前向传播里
    ...
    //可以先看一下data的数据结构
{
    ...
    int batch = net.batch;
    //注意n表示加载一次数据可以训练几次，其实就是subdivisions
