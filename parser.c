...
void load_convolutional_weights(layer l, FILE *fp)
{
    ...
    fread(l.biases, sizeof(float), l.n, fp);
    if (l.batch_normalize && (!l.dontloadscales)){
        //如果使用了batch normalization(参见知乎：深度学习中Batch NORMALIZATION为什么效果好？)就加载这三个参数
        fread(l.scales, sizeof(float), l.n, fp);
        fread(l.rolling_mean, sizeof(float), l.n, fp);
        fread(l.rolling_variance, sizeof(float), l.n, fp);
    }
...
void load_weights_upto(network *net, char *filename, int cutoff)
{
    ...
    //size_t fread(void *buffer, size_t size, size_t count, FILE *stream)。从一个文件流中读数据，最多读取count个项，每个项size个字节，如果调用成功返回实际读取到的项个数（小于或等于count），如果不成功或读到文件末尾返回0
    int transpose = (major > 1000) || (minor > 1000);

    int i;
    //coutoff顾名思义，之后的参数不加载,fine tuning的时候用
    for(i = 0; i < net->n && i < cutoff; ++i){
        //读取各层权重
        layer l = net->layers[i];
